...
## Seasonal
This is just a list, not function like cbep_colors().
```{r}
...
## Read Data
Note this version adjusts dates and times for Eastern Standard Time. It also assigns each observation to a season based on month of the year.
```{r load_data} 
...
1.  The colors have been reordered to simplify making the colors correspond to the same years as in the FOCB graphics and
...
```
## Seasonal
```{r ph_by_temp_seasonal, fig.width = 7, fig.height = 5}
plt <- ggplot(all_data, aes(temp, ph)) + 
